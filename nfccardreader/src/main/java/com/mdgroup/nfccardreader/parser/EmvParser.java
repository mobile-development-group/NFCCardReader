...
import com.mdgroup.nfccardreader.enums.CommandEnum;
import com.mdgroup.nfccardreader.enums.EmvCardScheme;
import com.mdgroup.nfccardreader.enums.SwEnum;
import com.mdgroup.nfccardreader.exception.CommunicationException;
import com.mdgroup.nfccardreader.iso7816emv.EmvTags;
import com.mdgroup.nfccardreader.iso7816emv.EmvTerminal;
import com.mdgroup.nfccardreader.iso7816emv.TLV;
import com.mdgroup.nfccardreader.iso7816emv.TagAndLength;
import com.mdgroup.nfccardreader.model.Afl;
import com.mdgroup.nfccardreader.model.EmvCard;
import com.mdgroup.nfccardreader.model.EmvTransactionRecord;
import com.mdgroup.nfccardreader.model.enums.CurrencyEnum;
import com.mdgroup.nfccardreader.utils.CommandApdu;
import com.mdgroup.nfccardreader.utils.ResponseUtils;
import com.mdgroup.nfccardreader.utils.TlvUtil;
import com.mdgroup.nfccardreader.utils.TrackUtils;
...
public class EmvParser {
    ...
    protected byte[] selectPaymentEnvironment() throws CommunicationException {
        LOGGER.d("Select " + (contactLess ? "PPSE" : "PSE") + " Application");
    ...
    protected byte[] parseFCIProprietaryTemplate(final byte[] pData) throws CommunicationException {
        ...
        LOGGER.d("(FCI) Issuer Discretionary Data is already present");
        return pData;
    }
    ...
    protected String extractApplicationLabel(final byte[] pData) {
        LOGGER.d("Extract Application label");
        String label = null;
    ...
    protected boolean readWithPSE() throws CommunicationException {
        boolean ret = false;
        LOGGER.d("Try to read card with Payment System Environment");
        // Select the PPSE or PSE directory
        ...
        if (ResponseUtils.isSucceed(data)) {
            ...
            LOGGER.d((contactLess ? "PPSE" : "PSE") + " not found -> Use kown AID");
        }
    ...
    protected void readWithAID() throws CommunicationException {
        LOGGER.d("Try to read card with AID");
    ...
    protected byte[] selectAID(final byte[] pAid) throws CommunicationException {
        LOGGER.d("Select AID: " + BytesUtils.bytesToString(pAid));
    ...
    protected boolean extractPublicData(final byte[] pAid, final String pApplicationLabel) throws CommunicationException {
        ...
        if (ResponseUtils.isSucceed(data)) {
            ...
            if (ret) {
                ...
                LOGGER.d("Application label:" + pApplicationLabel + " with Aid:" + aid);
                card.setAid(aid);
    ...
    protected EmvCardScheme findCardScheme(final String pAid, final String pCardNumber) {
        ...
        if (type == EmvCardScheme.CB) {
            ...
            if (type != null) {
                LOGGER.d("Real type:" + type.getName());
            }
        }
        return type;
    }
    ...
    protected byte[] getGetProcessingOptions(final byte[] pPdol, final IProvider pProvider) throws CommunicationException {
        ...
        try {
            ...
            LOGGER.e(ioe, "Construct GPO Command:" + ioe.getMessage());
        }
